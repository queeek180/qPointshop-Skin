...
function Player:PS_SetModel(Model)
  ...
  self:SetModel(Model)
	...
	local HandInfo = player_manager.TranslatePlayerHands(ModelName)
	local HandEnt = ents.Create("gmod_hands")
	
	if IsValid(HandEnt) then
		self:SetHands(HandEnt)
		HandEnt:SetOwner(self)
		HandEnt:SetModel(HandInfo.model)
		HandEnt:SetSkin(HandInfo.skin)
		HandEnt:SetBodyGroups(HandInfo.body)
