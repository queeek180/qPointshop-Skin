...
net.Receive('PS_SendItem', function(length, ply)
	...
	if other:PS_HasItem(item_id) then 
		ply:PS_Notify( other:Nick().." already has "..item_name.." in their inventory.")
		return
	end

	ply.PS_LastGaveItem = ply.PS_LastGaveItem or 0
	if ply.PS_LastGaveItem + 5 > CurTime() then
		ply:PS_Notify("Slow down! You can't give away items that fast.")
		return
	end

	if ply:PS_HasItemEquipped(item_id) then
		ply.PS_Items[item_id].Equipped = false
	...
	ply:PS_Notify("You gave ", other:Nick(), " ", item_name, " from your inventory.")

	other:PS_GiveItem(item_id)
	other:PS_Notify(ply:Nick(), " gave you ", item_name, " from their inventory.")
end)
...
util.AddNetworkString('PS_SetModel')
