...
function PROVIDER:GetData(ply, callback)
	...
	function q:onSuccess(data)
		if #data > 0 then
			...
			local redeemed = tobool(row.redeemed)

			callback(points, items, redeemed)
		else
			callback(0, {})
		end
	end
...
function PROVIDER:Redeemed(ply)
	local qs = [[
	INSERT INTO `pointshop_data` (uniqueid, points, items, redeemed)
	VALUES ('%s', '0', '[]', '%s')
	ON DUPLICATE KEY UPDATE 
		redeemed = VALUES(redeemed)
	]]
	qs = string.format(qs, ply:UniqueID(), 1)
	local q = db:query(qs)
